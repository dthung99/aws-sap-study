...
export function FlashcardView({ services, onBack }: FlashcardViewProps) {
  ...
  useEffect(() => {
    const handleKeyPress = (e: KeyboardEvent) => {
      ...
      if (e.key === "ArrowRight") {
        ...
        setCurrentIndex(
          (prev) => (prev - 1 + shuffled.length) % shuffled.length
        );
        setIsFlipped(false);
      } else if (e.key === " ") {
        e.preventDefault();
  ...
  const isMastered = progress.masteredServices.includes(
    currentService.serviceName
  );
  const isInReview = progress.reviewServices.includes(
    currentService.serviceName
  );
        ...
        <div className="bg-white rounded-lg p-4 shadow-lg text-center">
          <p className="text-sm font-bold text-gray-800">
            ...
            <span className="text-green-600">
              {progress.masteredServices.length}
            </span>{" "}
            | ⟳ Review:{" "}
            <span className="text-yellow-600">
              {progress.reviewServices.length}
            </span>
          </p>
        </div>
